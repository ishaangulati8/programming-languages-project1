...
function typeValidation(inputObj) {
    ...
    const schema = Joi.object({
        ...
        cancel: Joi.array().items(Joi.string().valid(...STOCK_SYMBOLS)).optional(),
    });
    const isValid = schema.validate(inputObj);
    if (isValid.error) {
        throw new Error(isValid.error);
    }
    const {buy = [], sell = [], cancel: []} = inputObj;
    if (!buy.length && !sell.length && !cancel.length) {
        throw new Error('ValidationError: "sell" or "buy" or "cancel" must be present.')
    }
}
